...
export default function InteractiveModule() {
    ...
    const triviaData = {
        onboarding: [
            ...
            {
                q: "¿Qué modalidad de transporte utilizarás mayormente?",
                options: ["Camión Pesado", "Furgoneta / Van", "Motocicleta", "Automóvil"],
                type: 'vehicle',
                tip: "Aris dice: Optimizaré los sensores de telemetría para tu tipo de vehículo."
            }
        ],
        truck: [
    ...
    const handleAnswer = (index) => {
        if (isOnboarding) {
            ...
            if (index === currentQuestions[triviaStep].correct) {
                setScore(score + 10);
            }
            setShowFeedback(true);
            setTimeout(() => {
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500 text-sm italic">
                        {isOnboarding ? "Configurando tu asistente personal..." : `Panel de ${userProfile.role} - ${userProfile.vehicle}`}
                    </p>
                </div>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {currentQuestions[triviaStep].options.map((opt, i) => (
                                    <button key={i} onClick={() => handleAnswer(i)} className="p-4 bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700/50 rounded-xl text-left text-sm transition-all hover:translate-x-1">
                                        {opt}
                                    </button>
                                ))}
                            </div>
